...
with tab_search:
    if is_de:
        st.subheader("Simuliere eine Dokumenten-Suche (Retrieval & Two-Stage Search)")
        st.markdown(f"""
        Wähle eine Domäne und ein Dokument aus. Wir vergleichen 3 Suchverfahren:
        1. **🔴 Original-Suche:** Exakter Scan im vollen {trained_I}-dimensionalen Raum.
        2. **🟡 Single-Stage TensoRAG:** Ultraschnelle Suche rein im komprimierten {trained_Q}-dimensionalen Unterraum.
        3. **🟢 Two-Stage TensoRAG (Coarse-to-Fine):** Schnelle Vorauswahl im Unterraum + exaktes Nachsortieren der Top-Kandidaten.
        """)
        lbl_dom_sel = "Domäne wählen"
        lbl_idx_sel = "Dokumenten-Index (Suchanfrage)"
        lbl_k_sel = "Anzahl der Top-Matches (k)"
        lbl_n_cand_sel = "Stufe 1 Over-Fetching Kandidaten (N)"
        lbl_title_orig = f"##### 🔴 Original (d = {trained_I})"
        lbl_title_comp = f"##### 🟡 Single-Stage (Q = {trained_Q})"
        lbl_title_two = f"##### 🟢 Two-Stage (Rescored)"
        col_doc_id = "Dokumenten-ID"
        col_cos_sim = "Ähnlichkeit"
        col_status = "Status"
        val_status_match = "✅ Match"
        val_status_near = "⚠️ Near"
        lbl_recall_single = "🟡 Single-Stage Recall: **{:.0f}%**"
        lbl_recall_two = "🟢 Two-Stage Recall: **{:.0f}%** (Perfekte Wiederherstellung!)"
        lbl_results_footer = """
        * **Single-Stage:** Bietet maximale Geschwindigkeit und RAM-Ersparnis (>90%).
        * **Two-Stage:** Kombiniert die 90%+ RAM-Ersparnis des komprimierten Index mit **100% exaktem Recall**, indem nur die Top-{} Kandidaten kurz rescored werden.
        """
    else:
        st.subheader("Simulate a Live Document Query (Retrieval & Two-Stage Search)")
        st.markdown(f"""
        Select a domain and document. We compare 3 retrieval strategies:
        1. **🔴 Original Search:** Exact scan in uncompressed {trained_I}-dimensional space.
        2. **🟡 Single-Stage TensoRAG:** Ultra-fast search directly in compressed {trained_Q}-dimensional subspace.
        3. **🟢 Two-Stage TensoRAG (Coarse-to-Fine):** Subspace coarse filtering + exact raw-data rescoring of top candidates.
        """)
        lbl_dom_sel = "Select Domain"
        lbl_idx_sel = "Query Document Index"
        lbl_k_sel = "Number of Top Matches (k)"
        lbl_n_cand_sel = "Stage 1 Over-Fetching Candidates (N)"
        lbl_title_orig = f"##### 🔴 Original (d = {trained_I})"
        lbl_title_comp = f"##### 🟡 Single-Stage (Q = {trained_Q})"
        lbl_title_two = f"##### 🟢 Two-Stage (Rescored)"
        col_doc_id = "Doc ID"
        col_cos_sim = "Similarity"
        col_status = "Status"
        val_status_match = "✅ Match"
        val_status_near = "⚠️ Near"
        lbl_recall_single = "🟡 Single-Stage Recall: **{:.0f}%**"
        lbl_recall_two = "🟢 Two-Stage Recall: **{:.0f}%** (Near 100% Recovery!)"
        lbl_results_footer = """
        * **Single-Stage:** Delivers maximum search speedup and >90% RAM reduction.
        * **Two-Stage:** Retains >90% index RAM savings while achieving **100% exact recall** by rescoring only top {} candidates.
        """
    ...
    with col_s1:
        ...
        top_n_candidates = st.slider(lbl_n_cand_sel, min_value=top_n, max_value=min(total_docs, 100), value=max(top_n*3, 20))
    ...
    with col_s2:
        ...
        orig_similarities = np.dot(db_vectors, query_vec) / (
            np.linalg.norm(db_vectors, axis=1) * np.linalg.norm(query_vec) + 1e-10
        )
        ...
        # 2. Single-Stage Compressed Space search
        ...
        comp_similarities = np.dot(db_vectors_proj, query_vec_proj) / (
            np.linalg.norm(db_vectors_proj, axis=1) * np.linalg.norm(query_vec_proj) + 1e-10
        )
        ...
        # 3. Two-Stage Coarse-to-Fine Search
        # Stage 1: Coarse filter candidates in Q-space
        cand_ids = np.argpartition(comp_similarities, -top_n_candidates)[-top_n_candidates:]
        # Stage 2: Rescore candidates on original raw vectors
        cand_vecs = db_vectors[cand_ids]
        rescored_sims = np.dot(cand_vecs, query_vec) / (
            np.linalg.norm(cand_vecs, axis=1) * np.linalg.norm(query_vec) + 1e-10
        )
        top_cand_order = np.argsort(rescored_sims)[::-1][:top_n]
        top_k_two_idx = cand_ids[top_cand_order]
        top_k_two_sims = rescored_sims[top_cand_order]
        
        # Recall calculations
        overlap_single = set(top_k_orig_idx).intersection(set(top_k_comp_idx))
        recall_single = (len(overlap_single) / top_n) * 100
        
        overlap_two = set(top_k_orig_idx).intersection(set(top_k_two_idx))
        recall_two = (len(overlap_two) / top_n) * 100
        
        # Display 3 side-by-side search result tables
        ...
        with col_res3:
            st.markdown(lbl_title_two)
            two_df = pd.DataFrame({
                col_doc_id: [f"Doc #{idx}" for idx in top_k_two_idx],
                col_cos_sim: [f"{s:.4f}" for s in top_k_two_sims],
                col_status: [val_status_match if idx in top_k_orig_idx else val_status_near for idx in top_k_two_idx]
            })
            st.dataframe(two_df, use_container_width=True)
            
        # Display recall progress bars
        c_rec1, c_rec2 = st.columns(2)
        with c_rec1:
            st.markdown(lbl_recall_single.format(recall_single))
            st.progress(recall_single / 100.0)
        with c_rec2:
            st.markdown(lbl_recall_two.format(recall_two))
            st.progress(recall_two / 100.0)
            
        st.markdown(lbl_results_footer.format(top_n_candidates))
